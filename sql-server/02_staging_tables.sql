
IF DB_ID(N'FSA_Staging') IS NOT NULL
BEGIN
    USE [FSA_Staging];

    -- Ensure schema exists
    IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'stg')
    BEGIN
        EXEC('CREATE SCHEMA [stg]');
    END

    BEGIN TRY
        CREATE TABLE [stg].[Claims]
        (
            claim_reference     NVARCHAR(100)  NOT NULL PRIMARY KEY,
            policy_reference    NVARCHAR(100)  NULL,
            claim_date          DATE           NULL,
            claim_type          NVARCHAR(50)   NULL,
            claim_status        NVARCHAR(50)   NULL,
            reserve_amount_gbp  DECIMAL(18,2)  NULL,
            paid_amount_gbp     DECIMAL(18,2)  NULL,
            handler_id          INT            NULL,
            days_open           INT            NULL,
            source_system       NVARCHAR(100)  NULL
        );
        PRINT N'Created table stg.Claims successfully'
    END TRY
    BEGIN CATCH
        SELECT 
            ERROR_NUMBER()   AS ErrorNumber,
            ERROR_SEVERITY() AS Severity,
            ERROR_STATE()    AS State,
            ERROR_PROCEDURE()AS ProcedureName,
            ERROR_LINE()     AS Line,
            ERROR_MESSAGE()  AS ErrorMessage;
    END CATCH;

    BEGIN TRY
        CREATE TABLE [stg].[GeneralLedgers]
        (
            gl_entry_id         NVARCHAR(100)  NOT NULL PRIMARY KEY,
            entry_date          DATE           NULL,
            gl_account_code     NVARCHAR(50)   NULL,
            account_description NVARCHAR(250)  NULL,
            entry_type          NVARCHAR(50)   NULL,
            debit_gbp           DECIMAL(18,2)  NULL,
            credit_gbp          DECIMAL(18,2)  NULL,
            policy_reference    NVARCHAR(100)  NULL,
            period              CHAR(7)        NULL, -- recommended format 'YYYY-MM'
            posted_by           NVARCHAR(100)  NULL,
            approved            CHAR(1)        NULL
        );
        PRINT N'Created table stg.GeneralLedger successfully'
    END TRY
    BEGIN CATCH
        SELECT
            ERROR_NUMBER()   AS ErrorNumber,
            ERROR_SEVERITY() AS Severity,
            ERROR_STATE()    AS State,
            ERROR_PROCEDURE()AS ProcedureName,
            ERROR_LINE()     AS Line,
            ERROR_MESSAGE()  AS ErrorMessage;
    END CATCH;

    BEGIN TRY
        CREATE TABLE [stg].[Payments]
        (
            payment_reference      NVARCHAR(100) NOT NULL PRIMARY KEY,
            policy_reference       NVARCHAR(100) NULL,
            payment_date           DATE NULL,
            payment_timestamp      DATETIME2(3) NULL,
            payment_method         NVARCHAR(50) NULL,
            payment_amount_gbp     DECIMAL(18,2) NULL,
            payment_status         NVARCHAR(50) NULL,
            bank_sort_code         NVARCHAR(20) NULL,
            bank_account_number    NVARCHAR(34) NULL,
            reconciled_flag        BIT NOT NULL CONSTRAINT DF_stg_Payments_Reconciled DEFAULT (0)
        );
        PRINT N'Created table stg.Payments successfully'
    END TRY
    BEGIN CATCH
        SELECT 
            ERROR_NUMBER()   AS ErrorNumber,
            ERROR_SEVERITY() AS Severity,
            ERROR_STATE()    AS State,
            ERROR_PROCEDURE() AS ProcedureName,
            ERROR_LINE()     AS Line,
            ERROR_MESSAGE()  AS ErrorMessage;
    END CATCH;

    -- create [stg].[Policies]

    BEGIN TRY
        CREATE TABLE [stg].[Policies]
        (
            policy_id                NVARCHAR(100)   NOT NULL PRIMARY KEY,
            underwriter_code         NVARCHAR(50)    NULL,
            policy_type              NVARCHAR(50)    NULL,
            risk_band                NVARCHAR(50)    NULL,
            region                   NVARCHAR(100)   NULL,
            agent_id                 NVARCHAR(100)   NULL,
            policy_start_date        DATE            NULL,
            policy_end_date          DATE            NULL,
            payment_frequency        NVARCHAR(50)    NULL,
            premium_amount           DECIMAL(18,2)   NULL,
            warehouse_premium_gbp    DECIMAL(18,2)   NULL,
            premium_variance_gbp     DECIMAL(18,2)   NULL,
            etl_status               NVARCHAR(50)    NULL,
            etl_processed_week       CHAR(7)         NULL, -- format 'YYYY-WW' recommended
            source_system            NVARCHAR(100)   NULL,
            data_quality_flag        BIT             NOT NULL CONSTRAINT DF_stg_Policies_DataQuality DEFAULT (0)
        );
        PRINT N'Created table stg.Policies successfully'
    END TRY
    BEGIN CATCH
        SELECT 
            ERROR_NUMBER()   AS ErrorNumber,
            ERROR_SEVERITY() AS Severity,
            ERROR_STATE()    AS State,
            ERROR_PROCEDURE()AS ProcedureName,
            ERROR_LINE()     AS Line,
            ERROR_MESSAGE()  AS ErrorMessage;
    END CATCH;

END
ELSE
BEGIN
    PRINT N'FSA_Staging Database does not exist'
END
